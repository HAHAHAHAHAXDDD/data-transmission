...
pub fn getBits(v1: Vec<f64>, tc: f64, tb: f64, fs: f64) -> Vec<i32>{
    let number_of_bits = tc/tb;
    let mut bits = vec![0;number_of_bits as usize];
    let mut idx: i32 = 0;
    let interval = (fs*tb)-1.;
    let mut checkpoint = interval;
    for i in 0..v1.len(){
        if i == checkpoint as usize{
            if v1[i] == 1.{
                bits[idx as usize] = 1;
                idx = idx+1;
...
fn main() {
    ...
    //wejscie
    let x = b.clone();
    println!("Wejsciowe bity: {:?}", x);
    println!("Wyjsciowe bity:");
    //ASK
    let tclone = t.clone();
    let bclone = b.clone();
    let ask = ASK(tclone, bclone, _fn, fs, a1, a2);
    let tclone = t.clone();
    let (_, _, dask) = dASK(ask, tclone, _fn, a1, fs, tb);
    //bity na wyjsciu
    let askbits = getBits(dask, tc, tb, fs);
    println!("\tASK: {:?}", askbits);
    
    //PSK
    let tclone = t.clone();
    let bclone = b.clone();
    let psk = PSK(tclone, bclone, _fn, fs);
    let tclone = t.clone();
    let (_, _, dpsk) = dPSK(psk, tclone, _fn, a1, fs, tb);
    //bity na wyjsciu
    let pskbits = getBits(dpsk, tc, tb, fs);
    println!("\tPSK: {:?}", pskbits);
    ...
    let tclone = t.clone();
    let (_, _, _, _, _, dfsk) = dFSK(fsk, tclone, _fn1, _fn2, a1, fs, tb);
    //bity na wyjsciu
    let fskbits = getBits(dfsk, tc, tb, fs);
    println!("\tFSK: {:?}", fskbits);
}
